...
Verifies that when vault and executor run on the same host, their data
directories are properly isolated via file permissions (chmod 700).

Tests require venya-vault and venya-executor system users to exist.
...
class TestCoLocationIsolation:
    ...
    def _skip_if_users_missing(self):
        ...
        if not self._check_user_exists(self.VAULT_USER):
            pytest.skip(f"{self.VAULT_USER} user not found")
    ...
    def test_vault_dir_inaccessible_by_executor(self):
        """venya-executor user must NOT read venya-vault data directory."""
        tmpdir = tempfile.mkdtemp()
        test_dir = Path(tmpdir) / "vault_data"
        test_dir.mkdir()
        test_file = test_dir / "secret.db"
        test_file.write_bytes(b"encrypted blob")

        vault_uid, vault_gid = self._get_uid_gid(self.VAULT_USER)
        executor_uid, executor_gid = self._get_uid_gid(self.EXECUTOR_USER)

        # Set ownership to venya-vault and permissions to 700
        result_chown = _sudo(["chown", "-R", f"{vault_uid}:{vault_gid}", str(test_dir)])
        ...
        assert stat.st_uid == vault_uid
        assert stat.st_gid == vault_gid
        assert oct(stat.st_mode)[-3:] == "700"

        # Try to read as executor
        result = _run_as_user(self.EXECUTOR_USER, ["cat", str(test_file)])
        assert result.returncode != 0, (
            f"executor ({self.EXECUTOR_USER}) should not read vault dir (mode 700)"
        )
    ...
    def test_executor_dir_inaccessible_by_vault(self):
        """venya-vault user must NOT read venya-executor data directory."""
        tmpdir = tempfile.mkdtemp()
        ...
        # Try to read as vault
        result = _run_as_user(self.VAULT_USER, ["cat", str(test_file)])
        assert result.returncode != 0, (
            f"vault ({self.VAULT_USER}) should not read executor dir (mode 700)"
        )
    ...
    def test_vault_dir_executable_by_vault(self):
        """venya-vault user MUST be able to access its own data directory."""
        tmpdir = tempfile.mkdtemp()
        ...
        # Verify vault can read
        result = _run_as_user(self.VAULT_USER, ["cat", str(test_file)])
        assert result.returncode == 0, "vault user must access its own dir"
